...
export default function AiBenefits() {
  return (
    <section
      id="benefits"
      className="app-container text-white bg-gradient-to-br from-dark to-gray-600 py-16 rounded-3xl h-3xl"
    >
      <div className="text-center mb-12">
        <span classNam="section-subheader">Ai Benefits</span>
        ...
        <p className="section-description text-gray-300">
          Discover the positive impact of AI across various sectors and industries.</p>
      </div>
      {/* bottom */}
        ...
        {benefits.map((benefit,i)=>{
            return(
                <article key={i} className="benefit-card">
                     <span className={`benefit-card-icon ${colorClasses[benefit.color]}`}>{benefit.icon}</span>
                     <h3 className="text-xl font-bold text-white">{benefit.title}</h3>
                     <p className="benefit-descriptionmt-2 text-gray-300">{benefit.description}</p>
                </article>
            )
        })}
      </div>
